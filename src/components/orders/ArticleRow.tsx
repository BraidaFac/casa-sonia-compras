...
} from "@/components/orders/ArticleAttributes";
...
export function ArticleRow({
  ...
  const [uploadingColors, setUploadingColors] = useState<Set<string>>(
    new Set(),
  );
  ...
  type PendingChange =
    | {
        type: "color";
        rowId: string;
        newColor: ColorValue | null;
        oldColorName: string;
      }
    | {
        type: "category";
        newCategory: ProductCategory;
        previousCategory: ProductCategory | null;
      };
  ...
  const lastConfirmedCategoryRef = useRef<ProductCategory | null>(
    article.category,
  );
  useEffect(() => {
    if (article.category) {
      ...
      p.colors.length > 0
        ? p.colors.map((color) => ({
            ...
            color: {
              ...
              colorBase:
                color.colorBase ||
                allColors.find((c) => c.id === color.id)?.colorBase ||
                "",
              hexColor:
                color.hexColor ||
                allColors.find((c) => c.id === color.id)?.hexColor ||
                "",
              isNew: false,
            },
    ...
    onChange({
      ...
      rows: article.rows.map((r) =>
        r.id === rowId
          ? {
              ...r,
              warehouseQuantities: { ...r.warehouseQuantities, [key]: val },
            }
          : r,
      ),
    });
  ...
  const articleHasQty = article.rows.some((r) => {
    if (selectedWarehouses.length > 0) {
      return Object.values(r.warehouseQuantities || {}).some(
        (v) => parseInt(v || "0", 10) > 0,
      );
    }
    return article.sizes.some(
      (s) => parseInt(r.quantities[s.name] || "0", 10) > 0,
    );
  });
  ...
  const localCoefNum =
    typeof localCoef === "number"
      ? localCoef
      : parseFloat(String(localCoef)) || 0;
  const precioConLocalCoef =
    costo > 0 && localCoefNum > 0 ? costo * localCoefNum : null;
  ...
  function handleColorChange(
    rowId: string,
    newColor: ColorValue | null,
    oldColor: ColorValue | null,
  ) {
    const oldColorName = oldColor?.name;
    const hasImages = oldColorName
      ? (article.colorImages[oldColorName] || []).some(
          (i) => !i.error && i.base64,
        )
      : false;
  ...
  function confirmPendingChange() {
    ...
    if (pendingChange.type === "color") {
      ...
      const clearedPrimaryColorNames = [
        ...(article.clearedPrimaryColorNames || []),
      ];
      ...
      if (
        primaryImg?.isFromOdoo &&
        !primaryImg.odooId &&
        !clearedPrimaryColorNames.includes(oldColorName)
      ) {
      ...
      >
        ...
        <div
          style={{
            ...
            marginBottom: 12,
            flexWrap: "wrap",
            ...(readOnly ? { pointerEvents: "none", userSelect: "none", opacity: 0.85 } : {}),
          }}
        >
            ...
            >
              <Combobox.Target>
                <TextInput
                  ...
                  size="xs"
                  style={{ width: "100%" }}
                  styles={{ input: { fontWeight: 600, color: "#ffffff", opacity: 1 }, label: { color: "#ffffff" } }}
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 4 }}>
            <Tooltip
              label="Este código ya existe en Odoo. No se puede confirmar la orden."
              withArrow
              color="red"
              disabled={!article.referenciaExistsInOdoo}
            >
              <TextInput
                ...
                w={140}
                disabled={readOnlyQuantities}
                styles={{ label: { color: "#ffffff" }, input: { color: "#ffffff", opacity: 1 } }}
                value={article.referencia}
                error={
                  article.referenciaExistsInOdoo
                    ? "Ya existe en Odoo"
                    : undefined
                }
                rightSection={isCheckingCode ? <Loader size={12} /> : undefined}
                onChange={(e) =>
                  onChange({
                    ...article,
                    referencia: e.currentTarget.value,
                    referenciaExistsInOdoo: false,
                  })
                }
                onBlur={async () => {
                  const code = article.referencia.trim();
                  if (!code || article.existingProductId) return;
                  setIsCheckingCode(true);
                  try {
                    const res = await fetch(
                      `/api/products/check-code?code=${encodeURIComponent(code)}`,
                    );
                    if (res.ok) {
                      const data = (await res.json()) as { exists: boolean };
                      onChange({
                        ...article,
                        referenciaExistsInOdoo: data.exists,
                      });
                    }
                  } catch {
                    // best-effort — no bloquear al usuario si falla
                  } finally {
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 4 }}>
            <NumberInput
              ...
              onChange={(val) =>
                onChange({ ...article, salePrice: String(val) })
              }
              description={
            ...
            {costo > 0 && (
              ...
              >
                <Popover.Target>
                  <Tooltip
                    label="Ajustar coeficiente y aplicar precio"
                    withArrow
                    disabled={coefPopoverOpen}
                  >
                    <ActionIcon
                      variant="subtle"
                      color="amber"
        ...
        >
          ...
          >
            ...
            <div style={readOnlyQuantities ? { pointerEvents: "none", userSelect: "none", opacity: 0.85 } : undefined}>
              ...
              <div style={{ overflowX: "auto" }}>
                ...
                >
                  <colgroup>
                    ...
                    />
                    <col
                      style={{ width: getColWidth("__color_base__", 110) }}
                    />
                    ...
                    {visibleSizes.map((size) => (
                      <col
                        key={size.id}
                        style={{
                          width: getColWidth(size.name, DEFAULT_SIZE_W),
                        }}
                      />
                    ))}
                    <col style={{ width: 60 }} />
                    ...
                    <tr>
                      ...
                      {visibleSizes.map((size) => {
                        ...
                        return (
                          ...
                          >
                            {!isOdooSize(realIdx) && !readOnlyQuantities && (
                              <ActionIcon
                                variant="subtle"
                                color="gray"
                                size={14}
                      ...
                      {!readOnly && (
                        <th style={{ ...headerCellStyle, pointerEvents: "all", opacity: 1 }}>
                          <button
                            type="button"
                            onClick={() => {
                              onOpenSizeModal?.();
                    ...
                    {article.rows.map((row) => {
                          ...
                          <tr key={row.id}>
                            ...
                            <td
                              style={{
                                ...
                                zIndex: 1,
                                background: "var(--surface)",
                                pointerEvents: !readOnly ? "auto" : undefined,
                              }}
                            >
                              <div
                                style={{
                                  ...
                                  <ColorProveedorCell
                                    ...
                                    usedColorKeys={
                                      new Set(
                                        article.rows
                                          .filter(
                                            (r) => r.id !== row.id && r.color,
                                          )
                                          .map((r) =>
                                            r.color!.id != null
                                              ? String(r.color!.id)
                                              : r.color!.name.toLowerCase(),
                                          ),
                                      )
                                    }
                                    onChange={(v) =>
                                      handleColorChange(row.id, v, row.color)
                                    }
                                  />
                                </div>
                                {!isOdooColor(row) && !readOnlyQuantities && (
                                  <ActionIcon
                                    variant="subtle"
                                    color="gray"
                                    size="xs"
                            ...
                            {/* Color Base */}
                            <td style={{ ...cellStyle, padding: 0, pointerEvents: !readOnly ? "auto" : undefined }}>
                              <ColorBaseCell
                                color={row.color}
                                colorBaseOptions={colorBaseOptions}
                                onChange={(updatedColor) =>
                                  updateRow(row.id, { color: updatedColor })
                                }
                              />
                            </td>
                            ...
                            {visibleSizes.map((size) => (
                              <td key={size.id} style={cellStyle}>
                                ...
                                {article.priceGranular &&
                                  (() => {
                                    const hasSpecific =
                                      !!row.prices?.[size.name];
                                    const hasFallback = !!article.price;
                                    const missing =
                                      !hasSpecific && !hasFallback;
                                    return (
                                      <input
                                        type="number"
                                        min={0}
                      ...
                      return (
                        <React.Fragment key={row.id}>
                          ...
                          {/* Fila de precios granulares — debajo de todas las sucursales */}
                          {article.priceGranular && (
                            <tr key={`${row.id}:prices`}>
                              <td
                                colSpan={4 + visiblePrintColumns.length}
                                style={{
                                  ...cellStyle,
                                  fontSize: 11,
                                  color: "var(--text3)",
              ...
              />
            </div>{/* end readOnlyQuantities wrapper */}

            {/* Add row — outside readOnlyQuantities so it works in edit mode on confirmed orders */}
            {!readOnly && (
              <Button
                variant="subtle"
                color="gray"
              ...
              <Stack gap="md">
                ...
                <div>
                  ...
                  {!article.category ? (
                    ...
                    <Text size="xs" c="dimmed">
                      Cargá colores en el tab Cantidades para poder subir
                      imágenes.
                    </Text>
                  ) : (
                    <Stack gap="sm">
                          ...
                          >
                            <Group justify="space-between" mb="xs">
                              ...
                              <Text size="xs" c="dimmed">
                                ...
                                {images.filter((i) => !i.error).length !== 1
                                  ? "es"
                                  : ""}
                              </Text>
                            </Group>
                                ...
                                {images.map((img) => (
                                  ...
                                  >
                                    ...
                                    <ActionIcon
                                      ...
                                      onClick={() =>
                                        handleRemoveImage(color.name, img.id)
                                      }
                                    >
                                      <X size={10} />
                                    </ActionIcon>
                ...
                <div>
                  ...
                  <Stack gap="xs">
                    ...
                    <Text size="xs" c="dimmed">
                      La descripción generada se publicará en la web de Odoo.
                      Podés editarla antes de confirmar la orden.
                    </Text>
                  </Stack>
                </div>
              </Stack>
